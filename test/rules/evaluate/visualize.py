...
# 生成句中实体的 svg
def display(
    doc: Doc, annotates: Any, idx: int, options: dict, desc: str | None = None
) -> None:
    true_ents = list()
...
def display_intention_core(
    doc: Doc, annotates: Any, idx: int, desc: str | None = None
):
    colors = {
        "Core": "#85C1E9",
    }
...
def is_different(
    true_entities, pred_entities, only_check: Sequence[str] | None = None
) -> bool:
    d1 = defaultdict(set)
    d2 = defaultdict(set)
