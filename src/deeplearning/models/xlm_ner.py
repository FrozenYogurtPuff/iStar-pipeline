...
class XLMSoftmaxForNer(XLMPreTrainedModel):
    def __init__(self, config):
        super(XLMSoftmaxForNer, self).__init__(config)
...
class XLMCrfForNer(XLMPreTrainedModel):
    def __init__(self, config):
        super(XLMCrfForNer, self).__init__(config)
        self.transformer = XLMModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
...
class XLMSpanForNer(XLMPreTrainedModel):
    def __init__(
        self,
        config,
    ):
        super(XLMSpanForNer, self).__init__(config)
        ...
        if self.soft_label:
            ...
            self.end_fc = PoolerEndLogits(
                config.hidden_size + 1, self.num_labels
            )
        self.init_weights()
    ...
    def forward(
        ...
        if start_positions is not None and self.training:
            ...
            if not self.soft_label:
                label_logits = (
                    torch.argmax(label_logits, -1).unsqueeze(2).float()
                )
